...
function printDialog () {
    if (currentTileMap == "HeroHouse" && fristTimeInHeroHouse && dialogIndex < currentDialogOrder.length) {
        if (currentDialogOrder[dialogIndex] == "mom") {
            mom.sayText(momDialogs[0], momDialogs[0].length * 100, true)
            momDialogs.removeAt(0)
        } else {
            hero.sayText(heroDialogs[0], heroDialogs[0].length * 100, true)
            heroDialogs.removeAt(0)
        }
        dialogIndex += 1
    } else if (currentTileMap == "Sea" && firstTimeInSea && dialogIndex < currentDialogOrder.length) {
    ...
    if (currentTileMap == "Village" && fristTimeInVillage && dialogIndex < currentDialogOrder.length) {
        ...
        dialogIndex ++
    } else if (currentTileMap == "Village" && fristTimeInVillage && dialogIndex > currentDialogOrder.length - 1) {
        villager.vx = -50
    } else if (currentTileMap == "Village" && fristTimeInVillage == false && dialogIndex < currentDialogOrder.length) {
        if (currentDialogOrder[dialogIndex] == "villager") {
            villager.sayText(villagerDialogs[0], villagerDialogs[0].length * 100, true)
            villagerDialogs.removeAt(0)
        } else {
        ...
        dialogIndex ++;
    } else if ( currentTileMap == "Village" && fristTimeInVillage == false && dialogIndex > currentDialogOrder.length - 1 && villagerFollowingPlayer == false){
        villager.follow(blueHouse, 50)
    }
    if (currentTileMap == "River" && fristTimeInRiver && dialogIndex < currentDialogOrder.length) {
        if (currentDialogOrder[dialogIndex] == "villager") {
...
scene.onHitWall(SpriteKind.Object, function(sprite: Sprite, location: tiles.Location) {
    if (currentTileMap == "Village"){
        villagerFollowingPlayer = true
        villager.follow(hero)
    }
})
scene.onOverlapTile(SpriteKind.Player, sprites.castle.tileDarkGrass1, function (sprite, location) {
    canEnterRaft = true
})
function enterRaftingModeExit () {
    ...
    tiles.placeOnRandomTile(raft, assets.tile`myTile47`)
    tiles.replaceAllTiles(assets.tile`myTile47`, assets.tile`myTile3`)
...
function loadRiver2 () {
    ...
    villagerDialogs = [
    "here see.. the trash is comming from here",
    "we clean the river",
    "but every time the trash",
    "comes back so we stopped cleaning",
    "Will you help us",
    "what is it?"
    ]
    heroDialogs = ["I think I have a plan", "lets go back to the village"]
...
function spawnVillagerExit () {
    villager = sprites.create(assets.image`villager2WalkFront1`, SpriteKind.Object)
    tiles.placeOnRandomTile(villager, assets.tile`myTile47`)
    tiles.coverAllTiles(assets.tile`myTile47`, assets.tile`myTile32`)
...
function spawnNet() {
    net = sprites.create(assets.image`Net`)
    tiles.placeOnRandomTile(net, assets.tile`myTile49`)
    tiles.coverAllTiles(assets.tile`myTile49`, assets.tile`myTile2`)
}
...
function loadVillage () {
    ...
    if (fristTimeInVillage) {
        ...
        heroDialogs = [
        "Hi, my name is Gregorius Eadburga.",
        "Do you live in this village?",
        "I've never found this village.",
        "I always went south to go fish.",
        "but today I decided to go north",
        "and I found a lot of trash.",
        "So I followed it here.",
        "Why is it so trashed here",
        "can you take me to the river"
        ]
        villagerDialogs = [
        "My name is Xavier Greyson nice to meet you.",
        "yaaa.",
        "So.. why are you here?",
        "haaa dont talk about it",
        "The river going through the city",
        "is connected here in the ocean",
        "but the people in the cities",
        "are throwing trash in to the river",
        "which contaminated the river",
        "now very little marine life",
        "exists in these rivers",
        "and we dont have a clean source of water",
        "everyone who drinks the water get sick",
        "come, follow me"
        ]
    } else {
        currentDialogOrder = ["hero", "villager"]
        heroDialogs = ["Do you have strong and durable nets"]
        villagerDialogs = ["Yes, wait here"]
    }
...
function spawnHeroExit () {
    hero = sprites.create(assets.image`Hero`, SpriteKind.Player)
    tiles.placeOnRandomTile(hero, assets.tile`myTile47`)
    tiles.coverAllTiles(assets.tile`myTile47`, assets.tile`myTile32`)
    tiles.coverAllTiles(assets.tile`myTile17`, sprites.castle.tileDarkGrass3)
    enterWalkingMode()
}
scene.onOverlapTile(SpriteKind.Player, assets.tile`myTile29`, function (sprite, location) {
    canEnterRaft = false
})
// scene.onOverlapTile(SpriteKind.Object, sprites.castle.tileDarkGrass2, function (sprite, location) {
// 	console.log("hello")
//     villager.follow(redHouse)
// })
function spawnMom () {
...
scene.onOverlapTile(SpriteKind.Object, sprites.castle.tileDarkGrass1, function (sprite, location) {
    canEnterRaft = false
})
function loadRiver () {
    if (fristTimeInRiver == false) {
        fristTimeInRiver2 = false
    }
    dialogIndex = 0
    currentDialogOrder = [
    "villager",
    ...
    villagerDialogs = ["This is the river", "next, when you find land ", "turn to the right"]
    heroDialogs = ["There is a lot of trash here"]
    ...
    sprites.destroyAllSpritesOfKind(SpriteKind.Object)
    canEnterRiver = false
    if (previousTileMap == "River2") {
        enterRaftingModeExit()
        previousTileMap = ""
    } else {
        enterRaftingModeWithVillager()
    }
...
function loadHeroHouse () {
    ...
    heroDialogs = ["Sooo...!", "ofcourse I love fishing.", "Ok mom."]
    momDialogs = ["Son we ran out of fish.", "Will you go fish some.", "Be carfull."]
...
function loadSea () {
    dialogIndex = 0
    currentDialogOrder = ["hero", "hero", "hero"]
    heroDialogs = ["There's a lot of trash here.", "I wonder where this leads too.", "Cant find any fish tooo."]
    currentTileMap = "Sea"
...
scene.onOverlapTile(SpriteKind.Raft, assets.tile`myTile44`, function (sprite, location) {
    ...
    if (currentTileMap == "River") {
        previousTileMap = "River"
        loadVillage()
    }
    if (currentTileMap == "River2") {
        previousTileMap = "River2"
        loadRiver()
    }
})
...
tiles.onMapUnloaded(function (tilemap2) {
    if (currentTileMap == "Sea") {
        fristTimeInHeroHouse = false
    }
    if (currentTileMap == "Village") {
        firstTimeInSea = false
    }
    if (currentTileMap == "River") {
        fristTimeInVillage = false
    }
    if (currentTileMap == "River2") {
        fristTimeInRiver = false
    }
})
...
let raftVel = 0
let heroVel = 0
let momDialogs: string[] = []
let villagerDialogs: string[] = []
let heroDialogs: string[] = []
let currentDialogOrder: string[] = []
let fristTimeInRiver2 = false
let fristTimeInRiver = false
let fristTimeInVillage = false
let firstTimeInSea = false
let fristTimeInHeroHouse = false
let canEnterRaft = false
let canEnterVillage = false
let currentTileMap = ""
let canSpawnNet = false
fristTimeInHeroHouse = true
firstTimeInSea = true
fristTimeInVillage = true
fristTimeInRiver = true
fristTimeInRiver2 = true
currentDialogOrder = ["placeHolder"]
heroDialogs = ["placeHolder"]
